...
%  - Replace the temp=0.3 invariance numbers only after you RE-RUN and SAVE
%    probe_score_quantization.py (see the marked spot in Sec. V-D).
...
steer the agent's severity assessment of a target vulnerability. Using a small,
freely deployable local model (Llama~3 8B) on a controlled synthetic-CVE testbed
that removes answer-leakage and memorised-prior confounds, we report three
findings. First, a single poisoned source reliably shifts the agent's severity
\emph{band} (e.g.\ \band{CRITICAL}$\rightarrow$\band{HIGH},
\band{MEDIUM}$\rightarrow$\band{CRITICAL}) in the attacker's intended direction
on 8/8 cases. Second, the attack is \emph{asymmetric}: inflation (manufacturing
a false emergency) is larger in magnitude than deflation (concealing a genuine
critical), and the model's own reasoning visibly resists deflation. Third,
independent corroboration is the hinge of both attack and defense: adding honest
sources both \emph{dilutes} the attack's magnitude (mean inflation shift
$2.0\rightarrow1.5$ bands with a third honest source) and \emph{enables} a
training-free leave-one-out internal-consistency check that detects the poisoned
source and restores the correct band with 8/8 detection, 8/8 recovery, and zero
false positives on honestly-disagreeing sources, under a stated $\geq$4-source
precondition. We additionally document, as a scoping finding, that the small
local model imitates CVSS output format without performing CVSS computation. We
frame this as a controlled mechanism study, not a benchmark, and outline the
extensions (multiple models, real CVEs, larger scale) that form a natural
research programme.
\end{abstract}
...
we measure the attack in both directions, vary the number of honest corroborating
sources, and evaluate a training-free consistency defense.
...
\item An \emph{inflation/deflation asymmetry}: inflation is larger-magnitude than
deflation; the model partially resists concealing a genuine critical
(\S\ref{sec:attack}, \S\ref{sec:discussion}).
...
defense that detects the poison and restores the correct band with zero false
positives, under a $\geq$4-source precondition (\S\ref{sec:dilution},
\S\ref{sec:defense}).
\item A \emph{scoping/instrument finding}: the small local model imitates CVSS
...
We are explicit about scope: this is a single-model, small-$n$, synthetic-testbed
...
and notably evaluates Llama~3 8B---the model we use---establishing that small
...
concrete training-free defense and characterise its precondition and ceiling.

\textbf{Our delta.} To our knowledge, the \emph{bidirectional asymmetry} of
single-source severity steering and the \emph{corroboration coupling} (the same
variable that dilutes the attack enables the defense) are not reported above. Our
...
brute-force cosine index; the chat model is Llama~3 8B~\cite{llama3} served locally via Ollama.
All components run locally and free of charge. Decoding is greedy (temperature 0)
with a fixed seed (1337); the model tag and digest are pinned and recorded in
...
All numbers are computed from saved logs (Llama~3 8B, pinned digest; BGE-M3
embeddings; seed 1337; temperature 0). $n=8$ synthetic CVEs (4 deflation, 4
inflation).

\subsection{Single-source steering and the asymmetry}\label{sec:attack}

With three honest sources present, a single poisoned source shifted the severity
band in the attacker's direction on \textbf{8/8} CVEs (Table~\ref{tab:attack}).
...
\begin{table}[t]
\centering
\caption{Attack with three honest sources. Band shift is signed in the attacker's
direction. Run \texttt{20260616T132519}.}
\label{tab:attack}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
...
Comparing a 2-honest-source corpus to the 3-honest-source corpus
...
\begin{table}[t]
\centering
\caption{Corroboration dilution. Mean $|$band shift$|$ by direction. The 2-source
run (\texttt{20260616T033459}) predates the band-shift field; shifts recomputed
from saved scores.}
\label{tab:dilution}
\small
\begin{tabular}{@{}lcc@{}}
...
3-honest-source corpus (4 sources per CVE under attack), achieved
...
\begin{table}[t]
\centering
\caption{Defense outcomes (leave-one-out internal-consistency). Run
\texttt{20260617T090409}. FP = honest source flagged on clean input.}
\label{tab:defense}
\small
...
% --- AUTHOR TODO: temperature invariance ---
% The temp=0.3 invariance (base_score = 9.8 on all 5 seeds, range 0.0, for both
% CVE-0001 and CVE-0002) was observed but NOT persisted to a log file. You stated
% you will RE-RUN and SAVE probe_score_quantization.py. After you do, insert the
% saved result here and cite the run directory. Until then, do NOT state it as a
% file-traceable result. Suggested sentence once saved:
% "Under non-greedy decoding (temperature 0.3, 5 seeds) the base score did not
%  vary (range 0.0), confirming the anchoring is not a greedy-decoding artifact
%  (run \texttt{<RUN_ID>})."
...
MITRE ATT\&CK technique mapping and found it unreliable with this model;
...
\textbf{Why deflation resists.} The model anchors on the honest sources' evident
severity and will partially, but not fully, follow a downplaying source---its
reasoning hedges rather than reverses. Inflation has no such anchor to fight. The
implication is double-edged: the less dangerous attack (false alarms) is the
easier one, while the more dangerous attack (concealment) is partially
resisted---but ``partially'' is not ``fully,'' and a one-band
\band{CRITICAL}$\rightarrow$\band{HIGH} drop can still de-prioritise an urgent
patch.

%==============================================================================
\section{Limitations}\label{sec:limitations}

\textbf{Scale.} $n=8$ synthetic CVEs: a controlled mechanism demonstration, not a
benchmark; the headline numbers characterise the mechanism on a small constructed
set, not population rates.

\textbf{Single model.} All results use Llama~3 8B. The asymmetry, the dilution,
and the defense may differ for other models; cross-model replication is the most
important next step.
...
\textbf{Band-level severity.} The model's crudeness forces band-level evaluation;
finer-grained (within-band) steering is unmeasurable here.

%==============================================================================
\section{Future Work}\label{sec:future}

The limitations above define a research programme: (1)~replicate across multiple
models and a larger synthetic set; (2)~add a real-CVE arm with honest fixtures
...
agent's CVE severity band, more easily toward inflation than deflation, with the
manipulation visible in the model's reasoning. The same independent corroboration
that dilutes the attack also enables a training-free leave-one-out consistency
check that detects the poison and restores the correct band with zero false
positives on honestly-disagreeing sources, given enough corroborating sources. On
a confound-free synthetic testbed and a crude-but-deployable local model, these
results are a controlled demonstration of a coupled attack/defense mechanism, and
a starting point for the scaling, generalisation, and real-world validation that
would establish its reach.
